...
def main() -> int:
    ...
    parser.add_argument("--skip-train", action="store_true", help="Skip Part 2 retraining")
    ...
    needs_retrain = args.retrain
    if not args.skip_train and model_meta_path.exists():
        import datetime
        with open(model_meta_path) as f:
            meta = json.load(f)
        trained_at = __import__("datetime").datetime.fromisoformat(meta.get("trained_at", "2000-01-01"))
        age_days = (datetime.datetime.now() - trained_at).days
        if age_days > 7:
            print(f"\n[Runner] Model is {age_days} days old — will retrain.")
            needs_retrain = True
        else:
            print(f"\n[Runner] Model is {age_days} days old — retraining not needed.")
    elif not model_meta_path.exists():
        needs_retrain = True
    ...
    for label in PIPELINE_ORDER:
        if label == "PART2" and args.skip_train:
            print(f"\n[Runner] Skipping {label} (--skip-train)")
            continue

        extra = []
        if label == "PART2" and args.model:
            extra = [f"--model={args.model}"]
